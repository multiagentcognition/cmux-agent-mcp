...
function printHelp(): void {
  console.log(`cmux-agent-mcp — terminal control plane for multi-agent AI workflows via CMUX

Usage:
  cmux-agent-mcp init [options]
  cmux-agent-mcp [server options]
  cmux-agent-mcp help

Commands:
  init      Register cmux-agent-mcp with all AI coding tools
  help      Show this help

When no command is given, the stdio MCP server starts (default behavior).

Examples:
  npx cmux-agent-mcp init              # global setup (all projects)
  npx cmux-agent-mcp init --project    # per-project setup (current dir)
  npx cmux-agent-mcp
`);
}
...
function runInitCli(args: string[]): void {
  ...
  if (parsed.values.help) {
    console.log(`Register cmux-agent-mcp with all AI coding tools
...
`);
  ...
  if (result.updatedFiles.length === 0) {
    console.log(`cmux-agent-mcp already configured — no changes needed.`);
    return;
  }

  const header = result.mode === 'global'
    ? 'cmux-agent-mcp registered globally for all projects'
    : `cmux-agent-mcp configured for ${result.projectRoot}`;
...
main().catch((err) => {
  console.error('cmux-agent-mcp failed:', err);
  process.exit(1);
});
