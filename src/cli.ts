...
function runInitCli(args: string[]): void {
  ...
  if (parsed.values.help) {
    console.log(`Register cmux-swarm with all AI coding tools
...
`);
  ...
  if (result.updatedFiles.length === 0) {
    console.log(`cmux-swarm already configured — no changes needed.`);
    return;
  }

  const header = result.mode === 'global'
    ? 'cmux-swarm registered globally for all projects'
    : `cmux-swarm configured for ${result.projectRoot}`;
